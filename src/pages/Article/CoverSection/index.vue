...
defineProps<{ article: ArticleMeta; loading: boolean }>()
</script>
  ...
  <div class="cover_info" :class="{ is_skeleton: loading }">
    ...
    <div v-if="!loading" class="article_header">
      <div class="tags" v-if="article.tags.length">
        <span v-for="tag in article.tags" :key="tag" class="tag hash"># {{ tag }}</span>
      </div>
      <h1 class="title">{{ article.title }}</h1>
      <div class="meta_info">
        <span class="meta_item" v-if="article.wordCount">
          <Icon class="icon" icon="lucide:file-text" />{{ article.wordCount }}
        </span>
        <span class="meta_item" v-if="article.readTime">
          <Icon class="icon" icon="lucide:clock" />{{ article.readTime }}分钟
        </span>
        <span class="meta_item" v-if="article.publishDate">
          <Icon class="icon" icon="lucide:calendar" />{{ article.publishDate }}
        </span>
